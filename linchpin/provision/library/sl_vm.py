...
#        username='DSW2001944', api_key='8a29bbe9fb49cd8f31873f988989abd018052892ef67cf8e6e8c6c60fa532649'
                                                            #))
except ImportError:
    HAS_SL = False
...
def create_virtual_instance(module):
    ...
    if instances:
        ...
        fd = open("/tmp/instan.txt","w")
        import json
        fd.write(json.dumps(instances))
        fd.close()
    ...
    if module.params.get('instance_id') is None and (module.params.get('tags') or module.params.get('hostname') or module.params.get('domain')):
        ...
        for instance in instances:
            try:
                ...
                vsManager.wait_for_transaction(instance['id'],
                                           module.params.get('wait_time')
                                           )
            except Exception:
                canceled = False
